...
const Modules = () => {
  ...
  const modules = [
    { id: 1, title: 'Points, Lines, and Angles', status: 'completed', image: 'twemoji:blue-book' },
    { id: 2, title: 'Triangles and the Pythagorean Theorem', status: 'unlocked', image: 'twemoji:green-book' },
    { id: 3, title: 'Polygons and Quadrilaterals', status: 'unlocked', image: 'twemoji:orange-book' },
    { id: 4, title: 'Circles and Symmetry', status: 'unlocked', image: 'twemoji:closed-book' },
    { id: 5, title: 'Introduction to 3D Shapes', status: 'unlocked', image: 'twemoji:ledger' },
  ];
    ...
    <div className="modules-page">
      <div className="modules-header glass-panel">
        <div className="header-content">
          ...
          <h2><AppIcon icon="twemoji:triangular-ruler" /> {courseTitle}</h2>
        </div>
        <p className="progress-text"><AppIcon icon="twemoji:star" /> Progress: {completedCount}/{totalCount} Modules Completed</p>
      </div>
        ...
        {modules.map((module, index) => (
          <div key={module.id} className="module-path-item">
            <div className={`module-card glass-panel ${module.status}`} onMouseEnter={() => triggerMascotVoice(module.title)}>
              <div className="module-icon"><AppIcon icon={module.image} /></div>
              <div className="module-content">
                ...
                {module.status === 'locked' ? (
                  ...
                  <button className="btn-primary" onClick={() => navigate('/notes', { state: { module: module.title } })}>
                    {module.status === 'completed' ? <><AppIcon icon="twemoji:check-mark-button" /> Review</> : <><AppIcon icon="twemoji:play-button" /> Continue Learning</>}
                  </button>
                )}
              </div>
            </div>
